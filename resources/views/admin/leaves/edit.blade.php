...
@section('content')
    @php
        ...
        $employeeName = $leaveRequest->employee?->user?->name
            ?: trim(($leaveRequest->employee->first_name ?? '').' '.($leaveRequest->employee->last_name ?? ''));
        $startDate = $leaveRequest->start_date ? \Illuminate\Support\Carbon::parse($leaveRequest->start_date)->format('Y-m-d') : null;
        $endDate = $leaveRequest->end_date ? \Illuminate\Support\Carbon::parse($leaveRequest->end_date)->format('Y-m-d') : null;
    @endphp

    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">Edit Leave Request</h1>
            <p class="text-body-secondary mb-0">Update {{ $employeeName ?: 'leave request #'.$leaveRequest->id }}.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.leaves.update', $leaveRequest) }}">
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <select id="employee_id" name="employee_id" class="form-select @error('employee_id') is-invalid @enderror" required>
                                ...
                                @foreach ($employees as $employee)
                                    ...
                                    @endphp
                                    <option value="{{ $employee->id }}" @selected((string) old('employee_id', $leaveRequest->employee_id) === (string) $employee->id)>
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <select id="leave_type_id" name="leave_type_id" class="form-select @error('leave_type_id') is-invalid @enderror" required>
                                ...
                                @foreach ($leaveTypes as $leaveType)
                                    <option value="{{ $leaveType->id }}" @selected((string) old('leave_type_id', $leaveRequest->leave_type_id) === (string) $leaveType->id)>
                    ...
                    @elseif ($hasLeaveType || $hasType)
                        <div class="col-12 col-lg-6">
                            ...
                            <input
                                ...
                                type="text"
                                name="{{ $typeColumn }}"
                                value="{{ old($typeColumn, $leaveRequest->{$typeColumn}) }}"
                ...
                @if ($hasRemarks || $hasNote)
                    <div class="mb-4">
                        ...
                        >{{ old($noteColumn, $leaveRequest->{$noteColumn}) }}</textarea>
                        @error($noteColumn)
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
