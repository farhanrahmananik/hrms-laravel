...
@section('content')
    @php
        ...
        $checkInValue = old($checkInColumn, $attendance->{$checkInColumn} ? \Illuminate\Support\Carbon::parse($attendance->{$checkInColumn})->format('Y-m-d\TH:i') : null);
        $checkOutValue = old($checkOutColumn, $attendance->{$checkOutColumn} ? \Illuminate\Support\Carbon::parse($attendance->{$checkOutColumn})->format('Y-m-d\TH:i') : null);
    @endphp

    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">Edit Attendance</h1>
            <p class="text-body-secondary mb-0">Update {{ $employeeName ?: 'attendance record #'.$attendance->id }}.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.attendance.update', $attendance) }}">
                    ...
                    @if ($hasAttendanceDate || $hasDate)
                        <div class="col-12 col-lg-6">
                            ...
                            <input
                                ...
                                type="date"
                                name="{{ $dateColumn }}"
                                value="{{ old($dateColumn, $attendance->{$dateColumn}?->format('Y-m-d') ?? $attendance->{$dateColumn}) }}"
                ...
                @if ($hasRemarks || $hasNote)
                    <div class="mb-4">
                        ...
                        >{{ old($noteColumn, $attendance->{$noteColumn}) }}</textarea>
                        @error($noteColumn)
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
