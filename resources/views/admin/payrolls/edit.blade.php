...
@section('content')
    @php
        ...
        $paymentDateValue = $paymentDateColumn && $payroll->{$paymentDateColumn}
            ? \Illuminate\Support\Carbon::parse($payroll->{$paymentDateColumn})->format($paymentDateColumn === 'paid_at' ? 'Y-m-d\TH:i' : 'Y-m-d')
            : null;
    @endphp

    <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1">Edit Payroll</h1>
            <p class="text-body-secondary mb-0">Update {{ $employeeName ?: 'payroll record #'.$payroll->id }}.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.payrolls.update', $payroll) }}">
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <select id="payroll_run_id" name="payroll_run_id" class="form-select @error('payroll_run_id') is-invalid @enderror" required>
                                ...
                                @foreach ($payrollRuns as $payrollRun)
                                    <option value="{{ $payrollRun->id }}" @selected((string) old('payroll_run_id', $payroll->payroll_run_id) === (string) $payrollRun->id)>
                    ...
                    @if ($monthColumn)
                        <div class="col-12 col-lg-6">
                            ...
                            <input
                                ...
                                name="{{ $monthColumn }}"
                                value="{{ old($monthColumn, $payroll->{$monthColumn}) }}"
                    ...
                    @if ($yearColumn)
                        <div class="col-12 col-lg-6">
                            ...
                            <input
                                ...
                                type="number"
                                name="{{ $yearColumn }}"
                                value="{{ old($yearColumn, $payroll->{$yearColumn}) }}"
                    ...
                    @if ($salaryColumn)
                        <div class="col-12 col-lg-4">
                            ...
                            <input
                                ...
                                min="0"
                                name="{{ $salaryColumn }}"
                                value="{{ old($salaryColumn, $payroll->{$salaryColumn}) }}"
                    ...
                    @if ($allowanceColumn)
                        <div class="col-12 col-lg-4">
                            ...
                            <input
                                ...
                                value="{{ old($allowanceColumn, $payroll->{$allowanceColumn}) }}"
                    ...
                    @if ($deductionColumn)
                        <div class="col-12 col-lg-4">
                            ...
                            <input
                                ...
                                value="{{ old($deductionColumn, $payroll->{$deductionColumn}) }}"
                    ...
                    @if ($statusColumn)
                        <div class="col-12 col-lg-4">
                            ...
                            <select id="{{ $statusColumn }}" name="{{ $statusColumn }}" class="form-select @error($statusColumn) is-invalid @enderror">
                                <option value="draft" @selected(old($statusColumn, $payroll->{$statusColumn}) === 'draft')>Draft</option>
                                <option value="pending" @selected(old($statusColumn, $payroll->{$statusColumn}) === 'pending')>Pending</option>
                                <option value="paid" @selected(old($statusColumn, $payroll->{$statusColumn}) === 'paid')>Paid</option>
                                <option value="approved" @selected(old($statusColumn, $payroll->{$statusColumn}) === 'approved')>Approved</option>
                                <option value="cancelled" @selected(old($statusColumn, $payroll->{$statusColumn}) === 'cancelled')>Cancelled</option>
                            </select>
                            @error($statusColumn)
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    @if ($paymentDateColumn)
                        <div class="col-12 col-lg-4">
                            ...
                            <input
                                ...
                                value="{{ old($paymentDateColumn, $paymentDateValue) }}"
                ...
                @if ($noteColumn)
                    <div class="mb-4">
                        ...
                        >{{ old($noteColumn, $payroll->{$noteColumn}) }}</textarea>
                        @error($noteColumn)
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
