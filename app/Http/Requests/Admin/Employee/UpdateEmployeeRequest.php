...
class UpdateEmployeeRequest extends FormRequest
{
    ...
    {
        return (bool) $this->user()?->hasPermission('employee.update');
    }
    ...
    public function rules(): array
    {
        ...
        if (Schema::hasColumn('employees', 'employee_code')) {
            $rules['employee_code'] = [
                ...
                'max:255',
                'alpha_dash:ascii',
                Rule::unique('employees', 'employee_code')->ignore($employee?->getKey()),
            ];
        }
        ...
        if (Schema::hasColumn('employees', 'code')) {
            $rules['code'] = [
                ...
                'max:255',
                'alpha_dash:ascii',
                Rule::unique('employees', 'code')->ignore($employee?->getKey()),
            ];
        }
        ...
        if (Schema::hasColumn('employees', 'slug')) {
            $rules['slug'] = [
                ...
                'max:255',
                'alpha_dash:ascii',
                Rule::unique('employees', 'slug')->ignore($employee?->getKey()),
            ];
        }
